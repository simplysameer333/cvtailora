...
import {
  FiUploadCloud, FiFile, FiX, FiBriefcase,
...
function UploadPageInner() {
  ...
  const hasLibrary  = (libraryCount ?? 0) > 0;
  ...
  return (
    <div className="grid grid-cols-1 xl:grid-cols-[minmax(0,1fr),380px] gap-6 items-start">
      ...
      {/* ── Hero ── (step is shown by the StepBar in the banner above) */}
      ...
      {/* ── Resume Library (shared component) ── */}
      <ResumeLibrary
        variant="picker"
        title="Your Resume Library"
        ctaLabel={isTailoring ? "Tailor with this" : "Use this resume"}
        onUseResume={handleUseLibraryResume}
        busyId={libraryLoadingId}
        onLoaded={setLibraryCount}
      />
      {hasLibrary && (
